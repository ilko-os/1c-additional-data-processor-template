...
1) `Управление настройками` - содержит команды для экспорта, импорта и очистки настроек. Cм. подробнее в разделе ["Экспорт, импорт и очистка настроек"](#Экспорт-импорт-и-очистка-настроек).
...
5)	(Необязательный) Добавить описание настроек ([см. подробнее](#описание-настроек-в-1сконфигуратор)). Место для изменения отмечено комментариями `// Начало области для модификации {{` и `// }} Окончание области для модификации`. Перечень процедур в модуле объекта обработки:
...
> [!NOTE] 
> Чтобы упростить обновления на новый релиз обработки, желательно следовать правилам из раздела  [рекомендации по разработке](#рекомендации-по-разработке).
...
> [!WARNING]  
> Файл с сохраненными настройками может содержать чувствительные данные. Нужно ответственно отноститься к его хранению и передаче. 
...
2) Для передачи параметров за пределы обработки, например, вызов процедуры или функции общего модуля служит глобальная переменная `КонтекстНастроекКоманды`. Прежнее наименование переменной `ПараметрыКоманды` оставлено для совместимости, считается устаревшим и будет удалено в новых релизах. Т.е. это упакованные переменные из [предыдущего пункта](#Доступ_к_настройкам_в_коде_1).
Полные пути к настройкам через переменную `КонтекстНастроекКоманды`:
	- `КонтекстНастроекКоманды.НастройкиОбработки`
	- `КонтекстНастроекКоманды.НастройкиОбработкиБезопасноеХранилище`
	- `КонтекстНастроекКоманды.НастройкиКоманды`
	- `КонтекстНастроекКоманды.НастройкиКомандыБезопасноеХранилище`

![Переменная_КонтекстНастроекКоманды](./img/004_Переменная_КонтекстНастроекКоманды.jpg)

![Переменная_ПараметрыКоманды_Устарела](./img/004_Переменная_ПараметрыКоманды.jpg)

3) Для передачи параметров за пределы обработки и доступа к настройкам **всех** команд и обработки служит глобальная переменная `КонтекстНастроекОбработки`. Прежнее наименование переменной `НастройкиКомандИОбработки` оставлено для совместимости, считается устаревшим и будет удалено в новых релизах.
	- Полные пути к настройкам через переменную `КонтекстНастроекОбработки`:
    	* `КонтекстНастроекОбработки.Обработка.НастройкиОбработки`
    	* `КонтекстНастроекОбработки.Обработка.НастройкиОбработкиБезопасноеХранилище`
    	* `КонтекстНастроекОбработки.Команды[ИдентификаторКоманды].НастройкиОбработки`
    	* `КонтекстНастроекОбработки.Команды[ИдентификаторКоманды].НастройкиОбработкиБезопасноеХранилище`
	- (Устаревший способ) Полные пути к настройкам через переменную `НастройкиКомандИОбработки`:
		* `НастройкиКомандИОбработки.НастройкиОбработки` 
		* `НастройкиКомандИОбработки.НастройкиОбработкиБезопасноеХранилище` 
		* `НастройкиКомандИОбработки[ИдентификаторКоманды].НастройкиКоманды` 
		* `НастройкиКомандИОбработки[ИдентификаторКоманды].НастройкиКомандыБезопасноеХранилище`  
	- где `ИдентификаторКоманды` - Строка - возможные значения описаны в функции `СведенияОВнешнейОбработке` в `Команда.Идентификатор`

![Переменная_КонтекстНастроекОбработки](./img/005_Переменная_КонтекстНастроекОбработки.jpg)

![Переменная_НастройкиКомандИОбработки_Устарела](./img/005_Переменная_НастройкиКомандИОбработки.jpg)
...
- Изменения вносить только в модуль объекта обработки
	...
	* в область `РазрешенныеРолиДляРедактированияНастроек`
...
> ![Infostart](./img/infostart-logo.svg)
>
> Статьи на Инфостарте:<br>
>
> - [Шаблон дополнительной обработки для конфигураций 1с на основе БСП с удобной системой хранения настроек](https://infostart.ru/1c/tools/2646650/)<br>
>
> 
