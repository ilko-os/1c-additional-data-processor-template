...
2)	В процедуре `СведенияОВнешнейОбработке` 
	* изменить значения для `ПараметрыРегистрации.Версия` и `ПараметрыРегистрации.Информация`.Место для изменения отмеченно комментариями `// Начало области для модификации {{` и `// }} Окончание области для модификации`
