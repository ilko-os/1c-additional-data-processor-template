...
- упростить просмотр, добавление и измение настроек
...
2)	`Режим загрузки настроек по умолчанию` - определяет правила загрузки настроек по умолчанию описанных в конфигураторе
	- `Обновление` - если настройка есть в табличной части, то обновляется только поле `Описание`, а поля `Тип` и `Значение` остаются без изменений. Если нет строки с параметром, то добавляется строка заполняются все поля из настроек, описанных в конфигураторе
...
5) `Идентификатор команды`
	...
	- управляет настроками для выбранной команды с помощью команд:
		...
		* `Сохранить настройки команды`
		Действие применятся к страницам `Настройки команды` и `Настройки команды (Безопасное хранилище)`
...
2)	В процедуре `СведенияОВнешнейОбработке` 
	* изменить значения для `ПараметрыРегистрации.Версия` и `ПараметрыРегистрации.Информация`
	* добавить (изменить) необходимые команды. Место для изменения отмеченно комментариями `// Начало области для модификации {{` и `// }} Окончание области для модификации`    
3)	В процедуре `ВыполнитьКоманду` описать реализацию выполнения команд из предыдущего пункта. Место для изменения отмеченно комментариями `// Начало области для модификации {{` и `// }} Окончание области для модификации`
...
- `Настройки команды безопасное хранилище` - "чувствительные" настройки, которые относятся к команде. Например, Например, `Логин` и `Пароль`, `КлючAPI` и т.д.  

К имени настройки (Колонка `Имя` в табличной части) применяются правила именования ключей структуры (переменных) встроеннго языка 1С:Предприятие. 
...
1) Добавить строку и задать `Имя`, должно соотвествовать правилам именования ключей структуры
...
Поддерживается загрузка нескольких занчений примитвного и ссылочного типа для контейнеров: 
- Список значений
- Массив
...
![Вставка значий из буфера обмена](./img/009_ЗагрузкаНесколькихЗначенийИзБуфера_2.jpg)
...
> [!IMPORTANT]
> Открывать нужно один и тотже файл обработки в режимах 1С:Конфигуратор 1С:Предприятие. 

Выполняется обработка и ее отладка из файла, а настройки из информационной базы обработки, выбранной в реквизите `Дополнительная обработка`.   
...
3. Скопировать из обработки на основе шаблона в обработку нового релиза шаблона:
	...
	* другие изменные участки кода, если были отклонения от рекомендаци
