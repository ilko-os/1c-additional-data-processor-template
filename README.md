...
2)  В процедуре `РазрешенныеРолиДляРедактированияНастроек` перечислить роли для которых разрешено открытие формы настроек обработки. Если у пользователя отсутствуют роли из этого перечня, то форма настроек не будет открывться. Место для изменения отмечено комментариями `// Начало области для модификации {{` и `// }} Окончание области для модификации`  
3)	В процедуре `СведенияОВнешнейОбработке` 
...
4)	В процедуре `ВыполнитьКоманду` описать реализацию выполнения команд из предыдущего пункта. Место для изменения отмечено комментариями `// Начало области для модификации {{` и `// }} Окончание области для модификации`
5)	(Необязательный) Добавить необходимы параметры в процедурах:
