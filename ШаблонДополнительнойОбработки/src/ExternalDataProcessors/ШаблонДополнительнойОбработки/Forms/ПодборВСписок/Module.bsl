...
Процедура ВставитьИзБуфераОбмена(Команда)
	...
	ПараметрыПоиска.Вставить("ОписаниеТипов", СписокЭлементов.ТипЗначения);
	//ПараметрыПоиска.Вставить("ПараметрыВыбора", Элементы.СписокЭлементов.ПараметрыВыбора);
...
Процедура ВставитьИзБуфераОбменаЗавершение(НайденныеОбъекты, ПараметрыВыполнения) Экспорт
	...
	Для Каждого Значение Из НайденныеОбъекты Цикл
		ОтчетыКлиентСервер.ДобавитьУникальноеЗначениеВСписок(СписокЭлементов, Значение, Неопределено, Истина);
	КонецЦикла;
