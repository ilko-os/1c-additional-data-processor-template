...
// Переменные для доступа к настройкам обработки из кода
...
Перем ПараметрыКоманды;    // Оставлена для совместимости, рекомендуется использовать `КонтекстНастроекКоманды`
Перем КонтекстНастроекКоманды;

Перем НастройкиКомандИОбработки;  // Оставлена для совместимости, рекомендуется использовать `КонтекстНастроекОбработки`
Перем КонтекстНастроекОбработки;
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Версия = "2026.5.14.1";
...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ПараметрыВыполнения) Экспорт
	...
	ПрочитатьНастройки(ИдентификаторКоманды, ОбработкаСсылка);
...
// 
// Параметры:
//  ТаблицаНастроек - ТаблицаЗначений - соответствует одноименному реквизиту на форме
//  РежимЗагрузкиНастроекПоУмолчанию - Строка - Определяет режим добавления строк: "Обновление", "ДобавлениеНовых", 
//		"Замещение"  
Процедура ЗагрузитьНастройкиОбработкиПоУмолчанию(ТаблицаНастроек, РежимЗагрузкиНастроекПоУмолчанию) Экспорт
	...
	ДобавитьСтрокуСНастройкой(ТаблицаНастроек, Имя, ТипКонтейнера, ТипЗначения, Значение, Описание,
...
Процедура ЗагрузитьНастройкиОбработкиБезопасноеХранилищеПоУмолчанию(ТаблицаНастроек,
	...
	ДобавитьСтрокуСНастройкой(ТаблицаНастроек, Имя, ТипКонтейнера, ТипЗначения, Значение,
...
//  РежимЗагрузкиНастроекПоУмолчанию - Строка - Определяет режим добавления строк: "Обновление", "ДобавлениеНовых", "Замещение" 
Процедура ЗагрузитьНастройкиКомандыПоУмолчанию(ТаблицаНастроек, ИдентификаторКоманды,
	РежимЗагрузкиНастроекПоУмолчанию) Экспорт
...
//  ИдентификаторКоманды - Строка - Идентификатор команды определенный в функции "СведенияОВнешнейОбработке()"
//  РежимЗагрузкиНастроекПоУмолчанию - Строка - Определяет режим добавления строк: "Обновление", "ДобавлениеНовых", "Замещение" 
Процедура ЗагрузитьНастройкиКомандыБезопасноеХранилищеПоУмолчанию(ТаблицаНастроек,
...
// Разрешенные роли для редактирования настроек.
// 
// Возвращаемое значение:
//  Массив - Строка - Разрешенные роли для редактирования настроек
Функция РазрешенныеРолиДляРедактированияНастроек() Экспорт
...
Процедура ПрочитатьНастройки(ИдентификаторКоманды, ОбработкаСсылка)
	
	НастройкиОбработки = Новый Структура;
	НастройкиОбработкиБезопасноеХранилище  = Новый Структура;
	
	
	НастройкиКомандИОбработки = Новый Структура; 	// Переменная оставлена для совместимости, считается устаревшей 
													// и будет удалена в новых релизах.
	КонтекстНастроекКоманды = Новый Структура;
	
	КонтекстНастроекОбработки = Новый Структура;
	КонтекстНастроекОбработки.Вставить("Обработка", Новый Структура);
	КонтекстНастроекОбработки.Вставить("Команды", Новый Структура);
	...
	Для Каждого Команда Из Сведения.Команды Цикл
		...
		Если Команда.Использование = "ВызовСерверногоМетода" Тогда
			...
			КомандаПустыеНастройки =  Новый Структура;
            КомандаПустыеНастройки.Вставить("НастройкиКоманды", Новый Структура);
			КомандаПустыеНастройки.Вставить("НастройкиКомандыБезопасноеХранилище", Новый Структура);

			КонтекстНастроекОбработки.Команды.Вставить(Команда.Идентификатор, КомандаПустыеНастройки);
		
			НастройкиКомандИОбработки.Вставить(Команда.Идентификатор, КомандаПустыеНастройки);
	...
	Если ТипЗнч(НастройкиИзХранилищаНастроекОбработки) = Тип("Структура") Тогда
		...
		Для Каждого Команда Из Сведения.Команды Цикл
			...
			Если Команда.Использование = "ВызовСерверногоМетода" Тогда
				...
				КонтекстНастроекОбработки.Команды[Команда.Идентификатор].Вставить("НастройкиКоманды", КомандаНастройкиКоманды);
	...
	Если ТипЗнч(НастройкиИзБезопасногоХранилища) = Тип("Структура") Тогда
		...
		Для Каждого Команда Из Сведения.Команды Цикл
			...
			Если Команда.Использование = "ВызовСерверногоМетода" Тогда
				...
				КонтекстНастроекОбработки.Команды[Команда.Идентификатор].Вставить("НастройкиКомандыБезопасноеХранилище",
	...
	НастройкиКоманды = КонтекстНастроекОбработки.Команды[ИдентификаторКоманды].НастройкиКоманды;
	НастройкиКомандыБезопасноеХранилище  = КонтекстНастроекОбработки.Команды[ИдентификаторКоманды].НастройкиКомандыБезопасноеХранилище;

	КонтекстНастроекКоманды.Вставить("НастройкиОбработки", НастройкиОбработки);
	КонтекстНастроекКоманды.Вставить("НастройкиОбработкиБезопасноеХранилище", НастройкиОбработкиБезопасноеХранилище);
	КонтекстНастроекКоманды.Вставить("НастройкиКоманды", НастройкиКоманды);
	КонтекстНастроекКоманды.Вставить("НастройкиКомандыБезопасноеХранилище", НастройкиКомандыБезопасноеХранилище);

	КонтекстНастроекОбработки.Обработка.Вставить("НастройкиОбработки", НастройкиОбработки);
	КонтекстНастроекОбработки.Обработка.Вставить("НастройкиОбработкиБезопасноеХранилище", НастройкиОбработкиБезопасноеХранилище);
	
		
	// // Переменная `ПараметрыКоманды` оставлена для совместимости, считается устаревшей и будет удалена в новых релизах.
	ПараметрыКоманды = КонтекстНастроекКоманды;
